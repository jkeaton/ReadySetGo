...
public class Dealer {
    ...
    String[] imageStrings = new String[81];
    ...
    private int picTest(){
        for(int i = 0; i < 81; i++){
            try{
                imageStrings[i] = (String.format("cardImage%d%d%d%d.png",cardCombos[i][0],cardCombos[i][1],cardCombos[i][2],cardCombos[i][3]));
            }
            catch(Exception e){
                e.printStackTrace();
                return -1;
    ...
    public void setNewDeck(){
        ...
        for(int i : shuffledCardNums){
            Card c = new Card(cardCombos[i-1][0],cardCombos[i-1][1],cardCombos[i-1][2],cardCombos[i-1][3],imageStrings[i-1]);
            currentDeck.add(c);
        }
    }
